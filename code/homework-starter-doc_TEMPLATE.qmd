...
drought_exp <- read_xlsx(path = here("data", 
                        "Valliere_etal_EcoApps_Data.xlsx"),
                         sheet = "First Harvest")
# here function refers you to a specific folder and pick a specific file within a sheet
...
**leaf dry weight is strongly positively correlated with total mass (Pearson's r is 0.46)**

**shoot mass is strongly positively correlated with total mass (Pearson's r is 0.902)**

**Root mass and total mass are strongly positively correlated (Pearson's r is 0.858)**
...
# reorder x axis to understand patterns better. mean doesn't show up but helps to reorder
...
**well-watered plants have higher biomass compared to drought stressed plants**
...
**total biomass is slightly positively correlated with specific leaf area, increasing as sla increases... moderate or strong depending on how you round**
...
**adding model 4, water treatment and species is the best model**
...
summary(model4)
```
total plant biomass = 0.055 + 0.117(water treatment) + 0.218(encelia californica) + 0.232(eschscholzia californica) + 0.313(grindelia camporum) + 0.229(nasella pulchra) + 0.050(penstemon centranthifolius) + 0.120(salvia leucophylla)

F(7, 62) = 27.08, p < 0.001, ⍺ = 0.05, adjusted R^2^ = 0.726

If species stays the same, well watered plants have a plant biomass that is 0.117 ± 0.017 grams greater in comparison to drought stressed plants.
...
-   SLA slope: **for each 1 unit increased in SLA (mm^2^/g), you expect a decrease in plant mass 0.0002 +- 0.0005 (figure out how to display +- in doc), NO EFFECT OF SLA ON MASS **\
-   well watered estimate: **On average, across species and SLA, well watered plants tend to be 0.12 +- 0.02g heavier (or more massive) than drought stressed plants. water treatment significantly predicts total mass**\
...
plot(model_preds, # model predictions
  ...
  labs(title = "Species and water treatment model visualization", # title of plot
       x = "Water treatment", # x-axis name
       y = "Total biomass (g)", # y-axis name
       color = "Species name") + # legend name
  theme(panel.grid = element_blank()) # getting rid of gridlines
```
...
model_preds_for_plotting <- model_preds %>% 
  rename(water_treatment = x,
         species_name = group) # renaming columns to make this easier to use
